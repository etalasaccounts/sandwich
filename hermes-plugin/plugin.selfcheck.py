...
def check_register_wires_skills_and_hook():
    ...
    assert len(ctx.registered_skills) == 5
...
def check_skill_paths_exist():
    assert set(plugin.SKILLS.keys()) == {"order", "prep", "status", "craft", "design"}
...
check("register() wires all five skills and the pre_llm_call hook", check_register_wires_skills_and_hook)
check("all five SKILL.md paths resolve to real files", check_skill_paths_exist)
