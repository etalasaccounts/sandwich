...
check("renderSpecMd renders header, scope, and checklist", () => {
...
check("renderSpecMd is pure content — no workflow instructions or generated-file disclaimer", () => {
  const md = renderSpecMd(validSpec(), 27);
  assert.ok(!md.includes("Mulai kerja"), "spec should not prescribe a workflow");
  assert.ok(!md.includes("superpowers:brainstorming"), "spec should not tell the reader what to run");
  assert.ok(!md.includes("/prep --done"), "spec should not tell the reader how to close out the feature");
  assert.ok(!md.includes("Generated by /prep"), "no disclaimer footer");
});
